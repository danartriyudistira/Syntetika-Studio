   ...
   for i in range(repeats):
      ...
      if random.random() < playChance:
         ...
         if quantize:
            ...
            quantizeShift = (round(syntetika.get_measure_time() * quantizeSubdivision) / quantizeSubdivision) - syntetika.get_measure_time()
         pos = i*loopLength + quantizeShift
         noteLength = length
         outputIndex = responseIndex
         if i == 0:
            outputIndex = 0
         canSyncopate = i > 0 and syntetika.get_step(32) % 4 == 0 or syntetika.get_step(32) % 4 == 3
