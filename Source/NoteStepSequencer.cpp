...
void NoteStepSequencer::SavePattern(int idx)
{
   if (idx < 0 || idx >= NSS_NUM_PATTERNS)
      return;
   ...
   juce::MemoryBlock block;
   {
      ...
      for (int i = 0; i < NSS_MAX_STEPS; ++i)
         out << mNoteLengths[i];
      out << mLength;
      out << static_cast<int>(mInterval);
      out << mOctave;
      out << static_cast<int>(mNoteMode);
      out << mLoopResetPoint;
      out << mNoteRange;
   }
...
void NoteStepSequencer::LoadPattern(int idx)
{
   if (idx < 0 || idx >= NSS_NUM_PATTERNS || !mPatterns[idx].mHasData)
      return;
   ...
   in >> mLength;
   {
      int interval;
      in >> interval;
      mInterval = static_cast<NoteInterval>(interval);
   }
   in >> mOctave;
   {
      int noteMode;
      in >> noteMode;
      mNoteMode = static_cast<NoteMode>(noteMode);
   }
   in >> mLoopResetPoint;
   in >> mNoteRange;
...
bool NoteStepSequencer::HasPattern(int idx)
{
   if (idx < 0 || idx >= NSS_NUM_PATTERNS)
      return false;
...
bool NoteStepSequencer::GetPatternData(int idx, std::vector<char>& out)
{
   if (idx < 0 || idx >= NSS_NUM_PATTERNS || !mPatterns[idx].mHasData)
      return false;
...
bool NoteStepSequencer::SetPatternData(int idx, const std::vector<char>& in)
{
   if (idx < 0 || idx >= NSS_NUM_PATTERNS)
      return false;
...
void NoteStepSequencer::SaveState(FileStreamOut& out)
{
   ...
   out << NSS_NUM_PATTERNS;
   for (int i = 0; i < NSS_NUM_PATTERNS; ++i)
   {
...
void NoteStepSequencer::LoadState(FileStreamIn& in, int rev)
{
   ...
   if (rev >= 4)
   {
      int numPatterns;
      in >> numPatterns;
      for (int i = 0; i < numPatterns && i < NSS_NUM_PATTERNS; ++i)
      {
