...
void VSTPlugin::GetVSTFileDesc(std::string vstName, juce::PluginDescription& desc)
{
   ...
   if (!found) //couldn't find the VST at this path. maybe its installation got moved, or the syntetika state was saved on a different computer. try to find a VST of the same name.
   {
...
void VSTPlugin::LoadVST(juce::PluginDescription desc)
{
   ...
   if (mPlugin != nullptr)
   {
      ...
      /*
         * For now, since Syntetika is at best stereo in stereo out,
...
#if SYNTETIKA_LINUX //HACK: weird race condition, which this seems to fix for now
   if (mPlugin == nullptr)
      return;
#endif
