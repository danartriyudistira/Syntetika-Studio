...
void OscController::oscMessageReceived(const juce::OSCMessage& msg)
{
   ...
   if (address.rfind("/syntetika/console", 0) == 0)
   {
      if (msg[0].isString())
   ...
   std::string control_prefix = "/syntetika/control/";
   std::string control_scaled_prefix = "/syntetika/control_scaled/";
   ...
   std::string module_prefix = "/syntetika/module/";
   ...
   if (
   (address.rfind("/note", 0) == 0 || address.rfind("/syntetika/note", 0) == 0) && msg.size() >= 2 &&
   ...
   if (address.rfind("/syntetika/location/recall", 0) == 0 && msg.size() == 1 && (msg[0].isInt32() || msg[0].isFloat32()))
   {
   ...
   if (address.rfind("/syntetika/location/store", 0) == 0 && msg.size() == 1 && (msg[0].isInt32() || msg[0].isFloat32()))
   {
      int number = msg[0].isInt32() ? msg[0].getInt32() : static_cast<int>(msg[0].getFloat32());
