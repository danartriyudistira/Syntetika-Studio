...
void ModularSynth::DumpStats(bool isCrash, void* crashContext)
{
   ...
   log.appendText("build: syntetika " + GetBuildInfoString() + ")\n");
}
...
void ModularSynth::Draw(void* vg)
{
   ...
   if (mFatalError != "")
   {
      ...
      DrawFallbackText(("syntetika " + GetBuildInfoString()).c_str(), 100, 50);
   ...
   {
      std::string loading("Syntetika is initializing audio...");
   ...
   {
      std::string loading("Syntetika is loading...");
      DrawTextNormal(loading, ofGetWidth() / 2 - GetStringWidth(loading, 28) / 2, ofGetHeight() / 2 - 6, 28);
      return;
   }
...
{
   mMainComponent->getTopLevelComponent()->setName("syntetika synth");
...
void ModularSynth::LoadLayout(ofxJSONElement json)
{
   ...
   //ofLoadURLAsync("http://syntetika.com/telemetry/"+jsonFile);
