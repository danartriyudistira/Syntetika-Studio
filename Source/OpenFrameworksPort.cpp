...
std::string ofToDataPath(const std::string& path)
{
   ...
   {
      auto defaultDataDir = juce::File::getSpecialLocation(juce::File::userDocumentsDirectory).getChildFile("Syntetika").getFullPathName().toStdString();
      auto dataDir = juce::SystemStats::getEnvironmentVariable("SYNTETIKA_DATA_DIR", defaultDataDir).toStdString();
#if SYNTETIKA_WINDOWS
...
std::string ofToFactoryPath(const std::string& subdir)
{
   ...
   {
      resDir = juce::File::getSpecialLocation(juce::File::SpecialLocationType::currentApplicationFile).getChildFile("../../share/Syntetika").getChildFile(subdir);
      if (!resDir.isDirectory())
         resDir = juce::File{ juce::CharPointer_UTF8{ Syntetika::CMAKE_INSTALL_PREFIX } }.getChildFile("share/Syntetika").getChildFile(subdir);
   }
#endif
#endif
   if (!resDir.isDirectory())
      throw std::runtime_error{ "Application directory not found. Please reinstall Syntetika." };
...
void ofSetColor(float r, float g, float b, float a)
{
   ...
   if (Push2Control::sDrawingPush2Display || gDrawingToFBO)
   {
