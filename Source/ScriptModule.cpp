...
#ifdef SYNTETIKA_PORTABLE_PYTHON
true;
#else
false;
#endif
...
void ScriptModule::InitializePythonIfNecessary()
{
...
#ifdef SYNTETIKA_PORTABLE_PYTHON
      py::exec(std::string{ "import sys; sys.executable = '" } + pythonHomeUtf8 + "/" SYNTETIKA_PORTABLE_PYTHON "'; del sys");
#endif
   ...
   {
      File(ofToDataPath("internal/python_" + std::string(Syntetika::PYTHON_VERSION) + "_installed")).create();
...
void ScriptModule::CheckIfPythonEverSuccessfullyInitialized()
{
   if (!sHasPythonEverSuccessfullyInitialized)
   {
      if (File(ofToDataPath("internal/python_" + std::string(Syntetika::PYTHON_VERSION) + "_installed")).existsAsFile())
         sHasPythonEverSuccessfullyInitialized = true;
   }
}
...
void ScriptModule::DrawModule()
{
   ...
   if (!sHasPythonEverSuccessfullyInitialized)
   {
        //DrawTextNormal("please ensure that you have Python 3.8 installed\nif you do not have Python 3.8 installed, Syntetika may crash\n\nclick to continue...", 20, 20);
      juce::String pythonVersionRev = Syntetika::PYTHON_VERSION;
      ...
      DrawTextNormal("this version of syntetika was built with Python " + pythonVersionRev.toStdString() + "\n" +
                     "please ensure that you have some flavor of Python " + pythonVersionMinor.toStdString() + " installed.\n" +
                     "(not an older or newer version!)\n\n" +
                     "if you do not, syntetika will crash!",
                     20, 20);
...
void ScriptWarningPopup::DrawModule()
{
   DrawTextNormal(std::string("") +
                  ...
                  "generally, these python scripts are innocuous, and used to do cool things within syntetika.\n" +
