...
public:
   ...
   , mVG(nullptr)
   {
      ofLog() << "syntetika synth " << GetBuildInfoString();

      // sigh ofLog isn't a stream so std::hex doesn't work so
      char jv[256];
      snprintf(jv, 255, "%x", JUCE_VERSION);
      ofLog() << "   juce version    : " << jv;
      ofLog() << "   python version  : " << Syntetika::PYTHON_VERSION;
#if SYNTETIKA_LINUX
      ofLog() << "   install prefix  : '" << Syntetika::CMAKE_INSTALL_PREFIX << "'";
#endif
      ofLog() << "   git hash        : " << Syntetika::GIT_HASH;
      ofLog() << "   git branch      : " << Syntetika::GIT_BRANCH;
      ofLog() << "   build time      : " << Syntetika::BUILD_DATE << " at " << Syntetika::BUILD_TIME;
      ofLog() << "   command line    : " << JUCEApplication::getCommandLineParameters();

      openGLContext.setOpenGLVersionRequired(juce::OpenGLContext::openGL3_2);
      openGLContext.setContinuousRepainting(false);
#if SYNTETIKA_LINUX //turning this on improves linux framerate, but seems to expose thread safety issues on windows/mac. see git PRs #349 and #396
      openGLContext.setComponentPaintingEnabled(false);
#endif

#ifndef SYNTETIKA_WINDOWS //windows crash handler is set up in ModularSynth() constructor
   ...
   void render() override
   {
      ...
      DrawBackgroundGrid(mVG, width, height);
   ...
   void DrawBackgroundGrid(NVGcontext* vg, float width, float height)
   {
      ...
      for (float y = 0; y < height; y += kGridSpacing)
      {
         nvgBeginPath(vg);
         nvgMoveTo(vg, 0, y);
...
private:
   ...
   bool keyPressed(const KeyPress& key) override
   {
       ...
       * always returns true whether or not Syntetika handles the event
