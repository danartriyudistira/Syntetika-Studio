...
void StepSequencer::SavePattern(int idx)
{
   if (idx < 0 || idx >= StepSequencer::kNumPatternSlots)
      return;

   juce::MemoryBlock block;
   {
      FileStreamOut out(block, false);
      mGrid->SaveState(out);
      ...
      out << mNumRows;
      out << mNumMeasures;
      out << static_cast<int>(mStepInterval);
      out << mStrength;
      out << static_cast<int>(mVelocityType);
   }

   mPatterns[idx].mData.resize(block.getSize());
   if (block.getSize() > 0)
      memcpy(mPatterns[idx].mData.data(), block.getData(), block.getSize());
   mPatterns[idx].mHasData = true;
}

void StepSequencer::LoadPattern(int idx)
{
   if (idx < 0 || idx >= StepSequencer::kNumPatternSlots || !mPatterns[idx].mHasData)
      return;

   juce::MemoryBlock block(mPatterns[idx].mData.data(), mPatterns[idx].mData.size());
   ...
   in >> mNumMeasures;
   {
      int interval;
      in >> interval;
      mStepInterval = static_cast<NoteInterval>(interval);
   }
   in >> mStrength;
   {
      int velType;
      in >> velType;
      mVelocityType = static_cast<StepVelocityType>(velType);
   }

   mGrid->SetGrid(mGrid->GetCols(), mNumRows);
   UpdateLights(true);
}

bool StepSequencer::HasPattern(int idx)
{
   if (idx < 0 || idx >= StepSequencer::kNumPatternSlots)
      return false;
   return mPatterns[idx].mHasData;
}

bool StepSequencer::GetPatternData(int idx, std::vector<char>& out)
{
   if (idx < 0 || idx >= StepSequencer::kNumPatternSlots || !mPatterns[idx].mHasData)
      return false;
   out = mPatterns[idx].mData;
   return true;
}

bool StepSequencer::SetPatternData(int idx, const std::vector<char>& in)
{
   if (idx < 0 || idx >= StepSequencer::kNumPatternSlots)
      return false;
   mPatterns[idx].mData = in;
   mPatterns[idx].mHasData = !in.empty();
   return true;
}
...
void StepSequencer::SaveState(FileStreamOut& out)
{
   ...
   out << StepSequencer::kNumPatternSlots;
   for (int i = 0; i < StepSequencer::kNumPatternSlots; ++i)
   {
      out << mPatterns[i].mHasData;
      if (mPatterns[i].mHasData)
      {
         int size = static_cast<int>(mPatterns[i].mData.size());
         out << size;
         out.WriteGeneric(mPatterns[i].mData.data(), size);
      }
   }
}
...
void StepSequencer::LoadState(FileStreamIn& in, int rev)
{
   ...
   if (rev >= 4)
   {
      int numPatterns;
      in >> numPatterns;
      for (int i = 0; i < numPatterns && i < StepSequencer::kNumPatternSlots; ++i)
      {
         in >> mPatterns[i].mHasData;
         if (mPatterns[i].mHasData)
         {
            int size;
            in >> size;
            mPatterns[i].mData.resize(size);
            in.ReadGeneric(mPatterns[i].mData.data(), size);
         }
      }
   }
}
