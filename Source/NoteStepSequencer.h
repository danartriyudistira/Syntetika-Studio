...
struct NoteStepSequencerPatternSlot
{
   bool mHasData{ false };
...
public:
   ...
   void SavePattern(int idx);
   void LoadPattern(int idx);
   bool HasPattern(int idx);
   bool GetPatternData(int idx, std::vector<char>& out);
   bool SetPatternData(int idx, const std::vector<char>& in);
...
private:
   ...
   NoteStepSequencerPatternSlot mPatterns[NSS_NUM_PATTERNS];
